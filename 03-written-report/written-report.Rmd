...
map_data <- life %>%
  mutate(Country = case_when(
         #adds US+Russia to map by renaming, can add more missing countries
...
ggplot(data = life_map, mapping = aes(long, lat, group = group)) +
  coord_fixed(1) +
  geom_polygon(aes(fill = as.numeric(`Life expectancy`)), color = "black") +
...
```{r model-comparison-adj-r-sq}
glance(backward_aic) %>% select(adj.r.squared) #this will become the selected model
glance(backward_bic) %>% select(adj.r.squared) 
glance(forward_aic) %>% select(adj.r.squared)
glance(forward_bic) %>% select(adj.r.squared)
