...
`Life expectancy`: Average life expectancy in years (Response variable)

`Country`: Name of country (Identifier variable)

`Region`: Area, subcontinent, or continent where a country is located (Possible predictor variable)

`Adult Mortality`: Probability of dying between 15 and 60 years per 1000 population (Possible predictor variable)
 
`infant deaths`: Number of infant deaths per 1000 population (Possible predictor variable)

`under-five deaths`: Number of under-five deaths per 1000 population (Possible predictor variable)

`Total expenditure`: General government expenditure on health as a percentage of total government expenditure (Possible predictor variable)

`Status`: Developed or developing status (Possible predictor variable)
...
p5 <- ggplot(data = by_status, aes(x = `Status`, y = `mean`)) +
  ...
  labs(x = "Status",
       ...
       subtitle = "Developed or developing")

(p1 + p3) / (p4 + p5)
```

In terms of the resposne variable, we see that life expectancy is left skewed with a center just over 70 years. Life expectancy ranges from about 50 years to 90 years. 

For relationships with predictor variables, we see that a strong negative relationship between life expectancy and adult mortality. We also a see a strange diversion in the points between countries that have lower and higher adult mortaltities. We predict that this is because some of those countries have higher infant mortalities, driving life expectancy down. We also see much higher life expectancies among countries with `status` "developed" than "developing" with a difference of about 12 years.
...
Here we can see the differences in life expectancy across the world. For example, it is clear that life expectancy is generally  higher in North American and European countries than African countries.
...
The goal of our analysis is to calculate the most precise prediction of the response variable (`Life expectancy`). To do this we will perform forward and backward selection using AIC and BIC. Forward selection entails adding variables recursively using AIC or BIC as the criterion while backward selection means dropping variables one at a time that are deemed irrelevant based on AIC or BIC.
...
```{r response-outliers, results = "hide"}
selected_model_aug %>%
  filter(.std.resid > 2 | .std.resid < -2) %>%
  left_join(life) %>%
  select(Country, `Standardized Residual` = .std.resid) %>%
  kable(caption = "Life Expectancy Outliers", digits = 3)
```
...
The standardized residuals do not display a discernible pattern when plotted against any of the quantitative variables. This combined with the similar lack of any relationship in the plot of standardized residuals vs. predicted values above indicates that the linearity assumption holds. 
...
selected_model_aug %>%
  ...
  kable(caption = "Predictor Variable Outliers", digits = 3)
```
...
The 12 countries in the table above have high leverage (leverage > leverage_threshold = 0.3230769) while no countries are influential (Cook's Distance > 0.5). 
...
`r r_sq`% of the variation in the `Life expectancy` is explained by the regression model above, which contains the predictor variables: `Region`, `Adult Mortality`, `infant deaths`, `Hepatitis B`, `Measles`, `under-five deaths`, `Total expenditure`, `Diphtheria`, `HIV/AIDS`, `Income composition of resources` and `Schooling`.

## Discussion + Conclusion

When looking at this selected model, a few things stand out. First, it was interesting to see the estimated coefficients of the different regions, which can be interpreted as the expected average life expectancy above the baseline of Asia (excluding Near East), holding all other predictors constant. The region of Sub-Saharan Africa had the lowest coefficient of -2.576 while the region Western Europe had the highest coefficient of 4.905. According to our model, these discrepancies are solely because of the difference in geographic region, which logically doesn't make much sense. Therefore, it's reasonable to assume that some information on economic, social, or health-related factors that differentiate geographic regions were missing from our data. If this project, were to be expanded, it would be important to include more than 23 potential variables in the full model.

Additionally, it is interesting to see that according to our model, a unit increase in infant deaths and Hepatitis B is expected to also increase life expectancy, on average (due to the positive estimated coefficients of these variables). This clearly doesn't make much sense. However, this can be taken with a grain of salt since the p-values of these variables are relatively high. This means that assuming the null hypothesis that these variables add no additional information to the model is true, the probability of observing a coefficient at least as extreme as the one we've observed is not super unlikely. We have a similar situation for Measles, which according to our model, adds no additional information to model even though it made it past all the model selection methods we conducted. 

Another interesting finding is the extremely high coefficient of `Income composition of resources`. With an estimated coefficient of 31.747, this means that for each additional unit of HDI (Human Development Index), we expect life expectancy to increase by 31.747 years, on average, holding all other predictor variables constant. However, this makes sense when it is understood that HDI is measured on a scale of 0 to 1, meaning that this outrageous increase will only be expected to happen if HDI goes all the way from 0 to 1. Therefore, it is more informative to interpret this variable in smaller units than than an increase by one, such as an increase by 0.01. In that case, for each additional 0.01 increase in HDI, we expect life expectancy to increase by about 0.32 years or about 4 months, on average, holding all other predictor variables constant.
