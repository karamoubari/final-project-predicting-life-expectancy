...
Also, it is strange to see that according to our model, a unit increase in infant deaths is expected to increase life expectancy, on average (due to the positive estimated coefficient of this variable). This clearly doesn't make much sense. However, this can be taken with a grain of salt since the p-values of is relatively high at 0.409. This means that assuming the null hypothesis that these variables' coefficients are equal to 0 is true, the probability of observing a coefficient at least as extreme as the one we've observed is not unlikely. We have a similar situation for Measles, which according to our model, adds no additional information to model even though it made it past all the model selection methods we conducted. 
